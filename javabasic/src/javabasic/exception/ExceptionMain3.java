...
public class ExceptionMain3 {
	...
	public static void main(String[] args) {
		...
		// 사용자정의예외를 생성해 두고 값이나 상황에 따라 알맞은 처리를 해주는 프로그래밍 기법
		// --> 예외상황에 대한 모듈화가 가능해짐 --> 적극적으로 활용하자!
		int age = 19;
		
		try {	
			// 나이가 15세 이하이면 Under15Exception 을 발생시킴
			if (age <= 15) throw new Under15Exception();
			// 나이가 19세 이하이면 Under19Exception 을 발생시킴
			else if(age <= 19) throw new Under19Exception();
		} catch (Under15Exception u15e) {
			System.out.println(u15e.toString()); // .tpString 생략되어있음	
