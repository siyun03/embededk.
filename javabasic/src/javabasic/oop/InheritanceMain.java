...
public class InheritanceMain {
	public static void main(String[] args) {
		...
		GrandParent parent = new Parent("홍아범",40,"산적");
		...
		//상위타입 > 하위타입 하위형변환
		//하위형변완이 되려면 한번은 상위형변환이 일어났어야 함
		//parent2 : parent 타입, 가지고있는 참조는 Parent
		//parent : GrandParent 타입 가지고 있는 참조는 Parent
		//				(상위형변환에 의해서 Parent참조를 가지게 됨)
		Parent parent2 = (Parent)parent;
		System.out.println(parent2.name + "님은 " + parent2.age + "살이고 직업은 " + parent2.job);
		
		//child2 :child 타입, 가지고있는 참조는 child
		//child : GrandParent 타입 가지고 있는 참조는 child
		...
		//각각의 하위타입들을 상위타입으로 호출하면.. 각각의 하위타입을 알 필요가 없다.
		//################하위타입의 개수가 아무리 많아도 상위타입으로 호출을 일원화 시킬 수 있다.###################
		GrandParent gp2 = p1;
		GrandParent gp3 = c1;
		Parent p2 = c1;
		gp2.say();
